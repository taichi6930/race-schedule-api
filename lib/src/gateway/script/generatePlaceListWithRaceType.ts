...
if (!validTargets.includes(target)) {
    throw new Error(
        'Usage: ts-node generatePlaceListWithRaceType.ts [keirin|boatrace|autorace]',
    );
}
...
function main(): void {
    ...
    // raceType列を追加
    const result: string[][] = [
        ['id', 'raceType', 'dateTime', 'location', 'updateDate'],
    ];
    ...
    console.log(
        `${target}/placeList.csv をraceType付きで上書き出力しました:`,
        outputPath,
    );
}
