...
@injectable()
export class WorldPlaceRepositoryFromStorageImpl
    implements IPlaceRepository<WorldPlaceEntity>
{
    ...
    public constructor(
        @inject('WorldPlaceS3Gateway')
        private readonly s3Gateway: IS3Gateway<WorldPlaceRecord>,
    ) {}
    ...
    public async fetchPlaceEntityList(
        ...
        // 年ごとの開催データを取得
        const placeRecordList: WorldPlaceRecord[] =
            await this.getPlaceRecordListFromS3();

        // Entityに変換
        const placeEntityList: WorldPlaceEntity[] = placeRecordList.map(
    ...
    public async registerPlaceEntityList(
        ...
        // 既に登録されているデータを取得する
        const existFetchPlaceRecordList: WorldPlaceRecord[] =
            await this.getPlaceRecordListFromS3();

        // PlaceEntityをPlaceRecordに変換する
        const placeRecordList: WorldPlaceRecord[] = placeEntityList.map(
     ...
     */
    @Logger
    private async getPlaceRecordListFromS3(): Promise<WorldPlaceRecord[]> {
        // S3からデータを取得する
        ...
        const placeRecordList: WorldPlaceRecord[] = lines
            .slice(1)
            .flatMap((line: string): WorldPlaceRecord[] => {
                try {
