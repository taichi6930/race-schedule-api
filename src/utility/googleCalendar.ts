...
export const toGoogleCalendarData = (
    ...
    const createDescription = (): string => {
        ...
        switch (raceEntity.raceData.raceType) {
            ...
            case RaceType.JRA: {
                const raceIdForNetkeiba = `${raceEntity.raceData.dateTime.getFullYear().toString()}${createNetkeibaBabacode(raceEntity.raceData.raceType, raceEntity.raceData.location)}${raceEntity.heldDayData.heldTimes.toXDigits(2)}${raceEntity.heldDayData.heldDayTimes.toXDigits(2)}${raceEntity.raceData.number.toXDigits(2)}`;
            ...
            case RaceType.NAR: {
                const raceIdForNetkeiba = `${raceEntity.raceData.dateTime.getFullYear().toString()}${createNetkeibaBabacode(raceEntity.raceData.raceType, raceEntity.raceData.location)}${raceEntity.raceData.dateTime.getXDigitMonth(2)}${raceEntity.raceData.dateTime.getDate().toXDigits(2)}${raceEntity.raceData.number.toXDigits(2)}`;
                return `距離: ${raceEntity.conditionData.surfaceType}${raceEntity.conditionData.distance.toString()}m
                ${raceTimeStr}
