...
describe.each(testRaceTypeListAll)(
    ...
    (raceType) => {
        ...
        it('|2|有効|有効|無効|Error|', () => {
            expect(() =>
                PlaceData.create(raceType, validDateTime, invalidLocation),
            ).toThrow();
        });
        ...
        it('|3|有効|無効|有効|Error|', () => {
            expect(() =>
                PlaceData.create(raceType, invalidDateTime, validLocation),
            ).toThrow();
        });
        ...
        it('|4|有効|有効|有効|copyで値変更OK|', () => {
            ...
            const copied = data.copy({ location: defaultLocation[raceType] });
